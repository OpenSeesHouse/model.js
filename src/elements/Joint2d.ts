...
export class Joint2D extends QuadElement {
  ...
  addToScene(scene: Scene, fac: number) {
    ...
    const corners = this.calculateCorners(fac);
  ...
  calculateCorners(fac: number): number[][] {
    const midpoints = this.nodes.map((node) => {
      let pnt1: THREE.Vector3 = new THREE.Vector3();
      pnt1.copy(node.position);
